...
int insert_data(const struct config *config, queue_t *data)
{
    ...
    while (ptr) {
        station = ptr->data;
        for (i = 0; station->ndata > 0 && i < 24; i++) {
            rs = station->rs[i];
            timestamp = generate_timestamp(rs.start);
            breaks = rs.breaks ? g_strdup_printf("%i", rs.breaks) : g_strdup("-");

            cmd = g_strdup_printf("SELECT insert_rbf_accelero('%s', '%s', %g, %g, %g, '%s')",
                                 station->name, timestamp, rs.data, rs.retx, rs.gap, breaks);
            res = PQexec(conn, cmd);
            g_free(breaks);
            g_free(cmd);
            g_free(timestamp);
    ...
    timestamp = generate_timestamp(rs.start);
    *(timestamp + 10) = 0;
    cmd = g_strdup_printf("SELECT func_insert_percentage_accelero('%s')", timestamp);
    res = PQexec(conn, cmd);
    g_free(timestamp);
    g_free(cmd);
